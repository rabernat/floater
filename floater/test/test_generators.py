...
def test_oceancoords(fs):
    ...
    #excludes Anirban's example to speed up the testing time 
    if fs.dx > 0.02:
        ...
        #rect grid test 
        float_x, float_y = np.transpose(fs.get_oceancoords(mesh='rect',mask=mask, mask_grid_x=mask_lon, mask_grid_y=mask_lat))
...
def test_to_mitgcm_format(fs):
    """Doesn't check actual values. Just makes sure the method runs without errors"""
    ...
    if fs.dx > 0.02:
        ...
        fs.to_mitgcm_format('dummy_test_file', tstart=0, mesh='hex', mask=mask, mask_grid_x=mask_lon, mask_grid_y=mask_lat)
        fs.to_mitgcm_format('dummy_test_file', tstart=0, mesh='rect', mask=mask, mask_grid_x=mask_lon, mask_grid_y=mask_lat)
